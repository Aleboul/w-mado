"""Extreme value copula module contains methods for sampling from an archimedean copula
and to compute the asymptotic variance of the lambda-FMadogram.

Extreme value copula are characterized by ther Pickands dependence funcrion which is a 
convex function and with $\lambda \vee (1-\lambda) \leq A(\lambda) \leq 1$. The copula
function

..math:: C(u,v) = (uv)^{A(log(v)/log(uv))}

is an extreme value copula. To sample from an extreme value copula, weed need the Pickands
dependence function and also its derivative. For each class, these two quantities will be
defined as methods.

Structure :

- Extreme value copula (:py:class:`Extreme`) from copy.bivariate.base.py
    - Hüsler and Reiss model (:py:class:`Husler_Reiss`)
    - Asymmetric logistic model (:py:class:`Asy_log`)
    - Asymmetric negative logistic model (:py:class:`Asy_neg_log`)
    - Asymmetric mixed (:py:class:`Asy_mix`)
    - t extreme value model (:py:class:`Student`)    
"""
...
class Asy_log(Extreme):
    """Class for asymmetric logistic copula model."""
