...
class Monte_Carlo(object):
	"""
		...
		--------
			...
			>>> n_iter, n_sample, p = 100, [128,256,512], p = [0.75, 0.75]
			>>> copula = extreme_value_copula.Asy_neg_log(theta = 10, psi1 = 0.5, psi2 = 1.0, n_sample = np.max(n_sample))
			>>> copula_miss = extreme_value_copula.Asy_log(theta = 2.0, psi1 = 1.0, psi2 = 1.0, n_sample = np.max(n_sample))
			>>> monte = Monte_Carlo(n_iter = n_iter, n_sample = n_sample, copula = copula, copula_miss = copula_miss)

			>>> x = np.linspace(0.01, 0.99, 50)
			>>> var = Monte.exec_varlmbd(lmbds = x, inv_cdf = norm.ppf, corr = False)
			>>> print(var) # show the empirical counterpart of the variance for a given grid of lambda.
	"""
	...
	def _fmado(self, X, miss, corr) :
		...
		------
			...
			corr : If true, return corrected version of lambda-FMadogram.
		Outputs
		-------
			lambda-FMadogram.
		"""
		...
		for p in range(0, Nnb):
			...
			V[:,p] = Femp
		if corr == True:
			FMado = (np.linalg.norm((np.power(V[:,0], 1/(1-self.lmbd)) - np.power(V[:,1], 1/self.lmbd))* (miss[0] * miss[1]), ord = 1) - (1-self.lmbd) * np.sum(np.power(V[:,0],1/(1-self.lmbd))*(miss[0] * miss[1])) - (self.lmbd) * np.sum(np.power(V[:,1],1/self.lmbd)* (miss[0] * miss[1]))) / (2 * np.sum(miss[0] * miss[1])) + 0.5 * (1-self.lmbd + np.power(self.lmbd,2)) / ((1+self.lmbd)*(1+1-self.lmbd))
		else :
			FMado = np.linalg.norm((np.power(V[:,0],1/(1-self.lmbd)) - np.power(V[:,1], 1/self.lmbd))* (miss[0] * miss[1]), ord = 1) / (2 * np.sum(miss[0] * miss[1]))
		...
		for k in range(self.n_iter):
			...
			for i in range(0,len(self.n_sample)):
				...
				if corr == "Both":
					...
					FMado_store[i], FMado_corr_store[i] = FMado[1], FMado[0]
				else :
	...
	def exec_varlmbd(self, lmbds, inv_cdf, n_lmbds = 50, corr = True):
		...
		Return
		------
			Give the empirical counterpart of the asymptotic variance of the lambda-FMadogram for a given grid of lambda.
		"""
		if lmbds is None :
